...
const int pmksa_cache_max_entries = 32;

struct rsn_pmksa_cache {
	struct rsn_pmksa_cache_entry *pmksa; /* PMKSA cache */
	int pmksa_count; /* number of entries in PMKSA cache */
	struct wpa_sm *sm; /* TODO: get rid of this reference(?) */
	void (*free_cb)(struct rsn_pmksa_cache_entry *entry, void *ctx, int replace);
	void *ctx;
};


#if 0
void pmksa_cache_set_expiration(struct rsn_pmksa_cache *pmksa);
...
void pmksa_cache_expire(void *eloop_ctx, void *timeout_ctx)
{
...
void pmksa_cache_reauth(void *eloop_ctx, void *timeout_ctx)
{
	struct rsn_pmksa_cache *pmksa = eloop_ctx;

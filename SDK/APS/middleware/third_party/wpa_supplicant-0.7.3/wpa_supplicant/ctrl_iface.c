...
int wpa_supplicant_ctrl_iface_preauth(struct wpa_supplicant *wpa_s,
					     char *addr)
{
	u8 bssid[ETH_ALEN];
...
int wpa_supplicant_ctrl_iface_ctrl_rsp(struct wpa_supplicant *wpa_s,
					      char *rsp)
{
#ifdef IEEE8021X_EAPOL
	char *pos, *id_pos;
...
int wpa_supplicant_ctrl_iface_bssid(struct wpa_supplicant *wpa_s,
					   char *cmd)
{
	char *pos;
//	int id;
...
char * wpa_supplicant_wps_ie_txt(char *pos, char *end,
...
int ctrl_iface_get_capability_pairwise(int res, char *strict,
...
int ctrl_iface_get_capability_group(int res, char *strict,
					   struct wpa_driver_capa *capa,
					   char *buf, size_t buflen)
{
	int ret, first = 1;
...
int ctrl_iface_get_capability_key_mgmt(int res, char *strict,
					      struct wpa_driver_capa *capa,
					      char *buf, size_t buflen)
{
	int ret;
...
int ctrl_iface_get_capability_proto(int res, char *strict,
					   struct wpa_driver_capa *capa,
					   char *buf, size_t buflen)
{
	int ret, first = 1;
...
int ctrl_iface_get_capability_auth_alg(int res, char *strict,
					      struct wpa_driver_capa *capa,
					      char *buf, size_t buflen)
{
	int ret, first = 1;
...
int wpa_supplicant_ctrl_iface_bss(struct wpa_supplicant *wpa_s,
...
int wpa_supplicant_ctrl_iface_ap_scan(
...
void wpa_supplicant_ctrl_iface_drop_sa(struct wpa_supplicant *wpa_s)
{
...
int wpa_supplicant_ctrl_iface_roam(struct wpa_supplicant *wpa_s,
					  char *addr)
{
	u8 bssid[ETH_ALEN];
	struct wpa_bss *bss;
...
int wpa_supplicant_global_iface_interfaces(struct wpa_global *global,
						  char *buf, int len)
{
	int res;
	char *pos, *end;
...
void wpa_free_iface_info(struct wpa_interface_info *iface)
{
...
int wpa_supplicant_global_iface_list(struct wpa_global *global,
					    char *buf, int len)
{
	int i, res;
...
#ifdef CONFIG_WPS_OOB
int wpa_supplicant_ctrl_iface_wps_oob(struct wpa_supplicant *wpa_s,
					     char *cmd)
{
...
int wpa_supplicant_ctrl_iface_wps_reg(struct wpa_supplicant *wpa_s,
					     char *cmd)
{
	u8 bssid[ETH_ALEN], *_bssid = bssid;
	char *pin;
...
#ifdef CONFIG_WPS_ER
int wpa_supplicant_ctrl_iface_wps_er_pin(struct wpa_supplicant *wpa_s,
						char *cmd)
{
	char *uuid = cmd, *pin;
	pin = os_strchr(uuid, ' ');
...
int wpa_supplicant_ctrl_iface_wps_er_learn(struct wpa_supplicant *wpa_s,
						  char *cmd)
{
	char *uuid = cmd, *pin;
	pin = os_strchr(uuid, ' ');
...
#ifdef CONFIG_WPS
char * wpa_supplicant_wps_ie_txt_buf(char *pos, char *end,
					    struct wpabuf *wps_ie)
{
	int ret;
	const char *txt;

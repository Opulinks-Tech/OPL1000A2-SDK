...
int wpa_config_validate_network(struct wpa_ssid *ssid, int line)
{
	int errors = 0;
...
struct wpa_ssid * wpa_config_read_network(FILE *f, int *line, int id)
{
...
struct wpa_config * wpa_config_read(const char *name)
{
    ...
    //Michael Liao debug, file system is not ready so we comment it, it should be support in the future when file system is ready
    /*
	f = fopen(name, "r");
	if (f == NULL) {
		os_free(config);
...
void write_str(FILE *f, const char *field, struct wpa_ssid *ssid)
{
...
void write_int(FILE *f, const char *field, int value, int def)
{
	if (value == def)
		return;
	fprintf(f, "\t%s=%d\n", field, value);
...
void write_bssid(FILE *f, struct wpa_ssid *ssid)
{
...
void write_psk(FILE *f, struct wpa_ssid *ssid)
{
...
void write_proto(FILE *f, struct wpa_ssid *ssid)
{
	char *value;
...
void write_key_mgmt(FILE *f, struct wpa_ssid *ssid)
{
	char *value;
...
void write_pairwise(FILE *f, struct wpa_ssid *ssid)
{
	char *value;
...
void write_group(FILE *f, struct wpa_ssid *ssid)
{
	char *value;
...
void write_auth_alg(FILE *f, struct wpa_ssid *ssid)
{
	char *value;
...
void write_eap(FILE *f, struct wpa_ssid *ssid)
{
	char *value;
...
void write_wep_key(FILE *f, int idx, struct wpa_ssid *ssid)
{
	char field[20], *value;
	int res;
...
void wpa_config_write_network(FILE *f, struct wpa_ssid *ssid)
{
	int i;
...
void wpa_config_write_global(FILE *f, struct wpa_config *config)
{
...
#ifndef CONFIG_NO_CONFIG_BLOBS
#if 0
struct wpa_config_blob * wpa_config_read_blob(FILE *f, int *line,
						     const char *name)
{
	struct wpa_config_blob *blob;
	char buf[256], *pos;
...
#if 0
int wpa_config_process_blob(struct wpa_config *config, FILE *f,
				   int *line, char *bname)
{
	char *name_end;
	struct wpa_config_blob *blob;
...
#if 0
int wpa_config_parse_int(const struct global_parse_data *data,
...
#if 0
int wpa_config_parse_str(const struct global_parse_data *data,
				struct wpa_config *config, int line,
				const char *pos)
{
	size_t len;
...
#if 0
int wpa_config_process_country(const struct global_parse_data *data,
				      struct wpa_config *config, int line,
				      const char *pos)
{
	if (!pos[0] || !pos[1]) {
...
#if 0
int wpa_config_process_load_dynamic_eap(
...
#ifdef CONFIG_WPS
#if 0
int wpa_config_process_uuid(const struct global_parse_data *data,
				   struct wpa_config *config, int line,
				   const char *pos)
{
	char buf[40];
...
int wpa_config_process_os_version(const struct global_parse_data *data,
					 struct wpa_config *config, int line,
					 const char *pos)
{
...
#if 0
const struct global_parse_data global_fields[] = {
#ifdef CONFIG_CTRL_IFACE
	{ STR(ctrl_interface) },
...
#if 0
#ifndef CONFIG_NO_CONFIG_BLOBS
int wpa_config_write_blob(FILE *f, struct wpa_config_blob *blob)
{

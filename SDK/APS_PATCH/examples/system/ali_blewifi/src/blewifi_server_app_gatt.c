...
#ifdef ALI_BLE_WIFI_PROVISION
static UINT16 gALISvcUuid               = BLE_UUID_AIS_SERVICE;

static UINT16 gALIReadCharUuid          = BLE_UUID_AIS_RC;
static UINT8  gALIReadCharVal[]         = CHAR_DECL_UUID16_ATTR_VAL(LE_GATT_CHAR_PROP_RD, BLE_UUID_AIS_RC);
static UINT8  gALIReadVal[2]            = {0};

static UINT16 gALIWriteCharUuid         = BLE_UUID_AIS_WC;
static UINT8  gALIWriteCharVal[]        = CHAR_DECL_UUID16_ATTR_VAL(LE_GATT_CHAR_PROP_RD | LE_GATT_CHAR_PROP_WR, BLE_UUID_AIS_WC);
static UINT8  gALIWriteVal[23];

static UINT16 gALIIndicateCharUuid      = BLE_UUID_AIS_IC;
static UINT8  gALIIndicateCharVal[]     = CHAR_DECL_UUID16_ATTR_VAL(LE_GATT_CHAR_PROP_RD | LE_GATT_CHAR_PROP_IND, BLE_UUID_AIS_IC);
static UINT8  gALIIndicateVal[16]       = {0};
static UINT16 gALIIndicateValClientCfg  = 2;

static UINT16 gALIWwnrcCharUuid         = BLE_UUID_AIS_WWNRC;
static UINT8  gALIWwnrcCharVal[]        = CHAR_DECL_UUID16_ATTR_VAL(LE_GATT_CHAR_PROP_RD | LE_GATT_CHAR_PROP_WR_NO_RESP, BLE_UUID_AIS_WWNRC);
static UINT8  gALIWwnrcVal[16];

static UINT16 gALINcCharUuid            = BLE_UUID_AIS_NC;
static UINT8  gALINcCharVal[]           = CHAR_DECL_UUID16_ATTR_VAL(LE_GATT_CHAR_PROP_RD | LE_GATT_CHAR_PROP_NTF, BLE_UUID_AIS_NC);
static UINT16 gALINcValClientCfg        = 1;
#endif
...
#ifdef ALI_BLE_WIFI_PROVISION
static LE_GATT_ATTR_T gALISvcDb[ALI_IDX_TOP] =
{
    ...
    [ALI_IDX_DATA_WRITE_VAL]        = CHARACTERISTIC_UUID16(&gALIWriteCharUuid, LE_GATT_PERMIT_READ|LE_GATT_PERMIT_AUTHOR_WRITE, sizeof(gALIWriteVal), 0, gALIWriteVal),
    // ALI Data ic Characteristic
    [ALI_IDX_DATA_INDICATE_CHAR]    = CHARACTERISTIC_DECL_UUID16(gALIIndicateCharVal),
    [ALI_IDX_DATA_INDICATE_VAL]     = CHARACTERISTIC_UUID16(&gALIIndicateCharUuid, LE_GATT_PERMIT_AUTHOR_READ, 512, sizeof(gALIIndicateVal), gALIIndicateVal),
    ...
    [ALI_IDX_DATA_NOTIFY_VAL]       = CHARACTERISTIC_UUID16(&gALINcCharUuid, LE_GATT_PERMIT_READ, 30, sizeof(gALIReadVal), gALIReadVal),
...
static LE_GATT_SERVICE_T *gAliSvc = 0;
#endif
...
static void BleWifi_Ble_AppHandleAliServiceWrite(LE_GATT_MSG_ACCESS_WRITE_IND_T *ind)
{
    ...
    BLEWIFI_INFO("APP - BleWifi_Ble_AppHandleAliServiceWrite attId = %d op = %x offset = %d len = %d\r\n", attrid, ind->flag, ind->offset, ind->len);
    ...
    switch (attrid)
    {
        case ALI_IDX_DATA_WRITE_VAL:
        {
            ...
            printf("\r\n");
#endif				
            LeGattChangeAttrVal(gAliSvc, ALI_IDX_DATA_WRITE_VAL, ind->len, ind->pVal);

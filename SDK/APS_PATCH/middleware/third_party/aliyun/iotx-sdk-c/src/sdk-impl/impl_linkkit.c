...
static int _iotx_linkkit_master_open(iotx_linkkit_dev_meta_info_t *meta_info)
{
    ...
    sdk_debug("_iotx_linkkit_master_open\n");
    if (ctx->is_opened) {
            sdk_debug("is_opened == 1\n");
        return FAIL_RETURN;
    }
    ctx->is_opened = 1;
    ...
    if (ctx->mutex == NULL) {
        sdk_err("Not Enough Memory");
        sdk_debug("HAL_MutexCreate fail\n");
        ctx->is_opened = 0;
        return FAIL_RETURN;
    }
    sdk_debug("_iotx_linkkit_master_open 1\n");
#ifdef DEVICE_MODEL_GATEWAY
    ...
    sdk_debug("_iotx_linkkit_master_open 2\n");
...
int IOT_Linkkit_Open(iotx_linkkit_dev_type_t dev_type, iotx_linkkit_dev_meta_info_t *meta_info)
{
    ...
    //memset((void*)&g_iotx_linkkit_ctx, 0, sizeof(g_iotx_linkkit_ctx));

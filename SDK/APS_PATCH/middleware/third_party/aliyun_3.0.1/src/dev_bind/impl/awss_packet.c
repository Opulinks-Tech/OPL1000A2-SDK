...
static void * awss_get_dev_info(void *dev_info, int len)
{
    ...
    snprintf(dev_info, len - 1, AWSS_DEV_INFO_FMT, AWSS_VER, pk, dev_name, mac_str, ip_str, bind_get_encrypt_type());
...
char *awss_build_sign_src(char *sign_src, int *sign_src_len)
{
    ...
    pk = os_zalloc_(IOTX_PRODUCT_KEY_LEN + 1);
    dev_name = os_zalloc_(IOTX_DEVICE_NAME_LEN + 1);
